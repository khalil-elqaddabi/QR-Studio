  ...
  return (
    <section aria-labelledby="type-heading" className="min-w-0">
      <div className="mb-2.5 flex items-baseline justify-between gap-3">
        ...
        <span className="text-xs text-ink-3">Pick what your code links to</span>
      </div>
      <div role="radiogroup" aria-label="QR type" className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-4">
        {GROUPS.map((group) => (
          <div key={group.id} className="col-span-full grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-4">
            <p className="col-span-full mb-0.5 flex items-center gap-2 pt-1 text-[11px] font-medium uppercase tracking-[0.12em] text-ink-3">
            ...
            {group.types.map((type) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'relative flex items-center gap-2 rounded-[10px] border px-2.5 py-2 text-left text-[13px] font-medium transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/35 active:scale-[0.98]',
                    active
